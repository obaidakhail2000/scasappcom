            ...
            <CardContent className="space-y-4">
              <div className="flex gap-2">
                {["Instagram", "Facebook", "SMS"].map((p) => (
                  <Button key={p} variant="outline" size="sm" className="gap-1.5">
                    {platformIcon(p)} {p}
                  </Button>
                ))}
              </div>
              <Textarea placeholder="Write your post content..." className="min-h-[120px] resize-none" />
              <div className="flex flex-col sm:flex-row gap-3">
                <Input type="datetime-local" className="sm:w-auto" />
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {scheduledPosts.map((p) => (
                <div key={p.id} className="p-3 rounded-lg bg-secondary space-y-2">
