import {
  LayoutDashboard, Star, Users, Megaphone, BarChart3, Table2, MessageSquareWarning, Settings, Image,
} from "lucide-react";
      ...
      <SidebarMenuItem key={item.title}>
        <SidebarMenuButton asChild isActive={isActive(item.url)}>
          <NavLink to={item.url} end
            className="rounded-xl transition-all duration-200 hover:bg-sidebar-accent/80 group/nav"
            activeClassName="bg-sidebar-primary/20 text-sidebar-primary font-semibold shadow-sm"
          >
            <item.icon className="mr-2.5 h-5 w-5 shrink-0 transition-colors group-hover/nav:text-sidebar-primary" />
    ...
    <Sidebar collapsible="icon" className="border-r-0">
      <SidebarHeader className="p-5 pb-6">
        <div className="flex items-center gap-3">
          <div className="h-11 w-11 rounded-xl bg-gradient-to-br from-sidebar-primary to-sidebar-primary/70 flex items-center justify-center shrink-0 shadow-lg shadow-sidebar-primary/20">
            <Star className="h-6 w-6 text-sidebar-primary-foreground" />
          </div>
          {!collapsed && (
            <div>
              <h2 className="text-sm font-bold text-sidebar-primary-foreground tracking-wide leading-tight">Meta Automation</h2>
              <p className="text-[10px] text-sidebar-foreground/50">Menu</p>
            </div>
          )}
        </div>
      </SidebarHeader>
      ...
      <SidebarContent className="px-2">
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu className="space-y-1">{renderItems(mainItems)}</SidebarMenu>
          </SidebarGroupContent>
        </SidebarGroup>
        ...
        <SidebarGroup className="mt-4">
          ...
          <SidebarGroupContent>
            <SidebarMenu className="space-y-1">{renderItems(manageItems)}</SidebarMenu>
          </SidebarGroupContent>
        </SidebarGroup>
      </SidebarContent>
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton asChild isActive={isActive("/settings")}>
              ...
              >
                <Settings className="mr-2.5 h-5 w-5 shrink-0" strokeWidth={1.8} />
